...
int				SocketTcpServer::accept(SocketTcpClient *&client)
{
...
{	
	...
	for (std::map<int, SocketTcpClient>::iterator cr = this->clients.begin();
			cr != this->clients.end(); cr++)
	{
...
int				SocketTcpServer::getClient(SocketTcpClient *&client, int clientSocket)
{
